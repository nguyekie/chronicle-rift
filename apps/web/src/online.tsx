...
export function EventHub() {
  ...
  useEffect(() => { api<any[]>('/events/active').then(async items => { setEvents(items); if (items[0]) setDetail(await api(`/events/${items[0].id}`)); }); }, []);
  if (!events[0]) return <p>Không có event đang hoạt động.</p>;
  const event = events[0];
  const complete = async (number: number) => { const result = await api<any>(`/events/${event.id}/stages/${number}/complete`, { method: 'POST' }); setMessage(`+${result.token} Eclipse Token`); setDetail(await api(`/events/${event.id}`)); };
  ...
  return <div className="event-hub"><div className="event-banner"><small>SỰ KIỆN GIỚI HẠN</small><h2>{event.name}</h2><p>{event.description}</p><b>{detail?.wallet.balance ?? 0} XU NHẬT THỰC</b></div><div className="event-stages">{Array.from({ length: 12 }, (_, index) => <button key={index} onClick={() => complete(index + 1)}>{index === 11 ? 'TRÙM' : index + 1}</button>)}</div><h3>Cửa hàng quy đổi</h3><div className="event-shop">{event.shopItems.map((item: any) => <article key={item.id}><b>{item.name}</b><span>{item.cost} Xu</span><button onClick={() => buy(item.id)}>QUY ĐỔI</button></article>)}</div><p>{message}</p></div>;
}
