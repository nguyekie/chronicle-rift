...
export function CampaignV2(){
 ...
 if(active)return <section className="chapter-battle"><button className="chapter-back" onClick={()=>setActive(null)}>← Trở lại bản đồ</button><div className="battle-law"><small>LUẬT CHIẾN TRƯỜNG</small><b>{laws[(active.number-1)%laws.length]}</b><span>{active.aiLevel==='BOSS'?'AI BOSS chiến thuật nâng cao':`AI cấp ${difficulty}`} · bộ bài tinh tuyển 30 lá · {boost.label}</span></div><BattleWithDeckSelection title={active.name} difficulty={difficulty} boss={active.aiLevel==='BOSS'} onComplete={()=>reward(active)} onBack={()=>setActive(null)}/></section>;
