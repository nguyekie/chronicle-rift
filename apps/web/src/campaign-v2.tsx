...
import {campaignBoost} from './campaign-balance';
import './campaign-v2.css';
import './campaign-layout-fix.css';
import './campaign-expansion.css';

type Stage={id:string;number:number;name:string;kind:string;aiLevel:string;enemyDeck?:{mutation?:string;strategy?:string;powerTier?:number};rewardGold:number;rewardDust:number;unlocked:boolean;progress?:{stars:number}|null};
...
const mutationNames:Record<string,string>={MANA_INVERSION:'Đảo Năng Lượng',FATE_SURGE:'Bão Vận Mệnh',FORTIFIED_ROWS:'Phòng Tuyến Bất Diệt',ARCANE_ECLIPSE:'Nhật Thực Ma Pháp',DOUBLE_FORESEE:'Song Trùng Tiên Kiến',FIRST_REBIRTH:'Hồi Sinh Đầu Tiên',TURN_COUNTDOWN:'Đồng Hồ Tận Diệt',MIRROR_CAST:'Gương Phản Phép',CELESTIAL_TAX:'Thuế Thần Linh',VOID_TIDE:'Thủy Triều Hư Không',SUDDEN_DEATH:'Tử Chiến',REALITY_COLLAPSE:'Thực Tại Sụp Đổ'};
const stageLaw=(stage:Stage,index:number)=>stage.enemyDeck?.mutation?`${mutationNames[stage.enemyDeck.mutation]??stage.enemyDeck.mutation}: luật đặc biệt cấp Thần Vực.`:laws[index%laws.length];
...
export function CampaignV2(){
 ...
 const difficulty=((chapter?.number??1)-1)*12+(active?.number??1);
 const boost=campaignBoost(chapter?.number??1,active?.number??1,active?.aiLevel==='BOSS');
 if(active)return <section className="chapter-battle"><button className="chapter-back" onClick={()=>setActive(null)}>← Trở lại bản đồ</button><div className="battle-law"><small>LUẬT CHIẾN TRƯỜNG</small><b>{stageLaw(active,active.number-1)}</b><span>{active.aiLevel==='BOSS'?'AI BOSS chiến thuật nâng cao':`AI cấp ${difficulty}`} · bộ bài tinh tuyển 30 lá · {boost.label}</span></div><BattleWithDeckSelection title={active.name} difficulty={difficulty} boss={active.aiLevel==='BOSS'} onComplete={()=>reward(active)} onBack={()=>setActive(null)}/></section>;
