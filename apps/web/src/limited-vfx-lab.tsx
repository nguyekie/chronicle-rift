...
export function LimitedVfxLab(){
  ...
  const[active,setActive]=useState<{key:number;slug:string;label:string;picture:ReturnType<typeof cardArt>}|null>(null);
  const[auto,setAuto]=useState(false),[cursor,setCursor]=useState(0);
  const play=(index:number)=>{const card=limited[index];if(!card)return;const effect=effects[card.code];if(!effect)return;setCursor(index);setActive({key:Date.now(),slug:effect.slug,label:effect.label,picture:cardArt(card.name,card.code)});window.setTimeout(()=>setActive(null),3200)};
  useEffect(()=>{if(!auto)return;play(cursor);const timer=window.setInterval(()=>setCursor(value=>(value+1)%limited.length),3500);return()=>window.clearInterval(timer)},[auto]);
  ...
  return <main className="vfx-lab">
    ...
    {active&&<div key={active.key} className={`limited-cinematic limited-${active.slug} owner-keeper`}><i/><i/><i/><span className={`limited-card-sigil ${active.picture.className}`} style={active.picture.style}/>{active.slug==='eclipse-rider'&&<video className="limited-alpha-video" autoPlay muted playsInline preload="auto"><source src="/vfx/eclipse-rider.webm" type="video/webm"/></video>}<span className="limited-particles">{Array.from({length:18},(_,index)=><i key={index}/>)}</span><div><small>THỬ NGHIỆM HIỆU ỨNG GIỚI HẠN</small><b>{active.label}</b></div></div>}
  </main>
}
