...
function knightDragon(scene:THREE.Scene){
 ...
 const wings=[-1,1].map(side=>{const wing=new THREE.Group();wing.position.set(-.2,.25,side*.55);dragon.add(wing);const shape=new THREE.Shape();shape.moveTo(0,0);shape.lineTo(-1.8,side*.15);shape.lineTo(-.4,side*1.9);shape.closePath();const g=new THREE.ShapeGeometry(shape);const w=mesh(g,new THREE.MeshStandardMaterial({color:0x16243c,side:THREE.DoubleSide,transparent:true,opacity:.88}),wing,[0,0,0],[Math.PI/2,0,0]);return wing});
