import{describe,it,expect}from'vitest';import{nextPity,pickRarity,pickPackRarity,ratesFor,craftCost,dismantleValue,limitedRatePerPack}from'./pack-logic.js';
describe('pack economy',()=>{
 ...
 it('uses different rates for each pack',()=>{expect(ratesFor('STARTER').COMMON).toBe(70);expect(ratesFor('RIFT_SEAL').LEGENDARY).toBe(5);expect(ratesFor('RIFT_SEAL').LIMITED).toBe(0);expect(limitedRatePerPack).toBe(.5);expect(ratesFor('IRONVALE').EPIC).toBe(6)});
