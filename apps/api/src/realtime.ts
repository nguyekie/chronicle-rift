...
export function attachRealtime(http:HttpServer){
 ...
 io.on('connection',socket=>{
  ...
  socket.on('disconnect',async()=>{removeFromQueues(userId);for(const[code,room]of rooms)if(room.host.userId===userId||room.host.socketId===socket.id)rooms.delete(code);for(const[matchId,session]of sessions)if(session.users.includes(userId)){await db.matchPlayer.updateMany({where:{matchId,userId},data:{disconnectedAt:new Date()}});setTimeout(async()=>{const sockets=await io.in(matchId).fetchSockets();if(!sockets.some(client=>client.data.userId===userId)&&session.state.phase!=='ENDED'){const result=applyAction(session.state,{type:'CONCEDE',playerId:userId});if(result.ok){session.state=result.state;io.to(matchId).emit('match:end',{winnerId:session.state.winnerId,reason:'DISCONNECT_TIMEOUT'})}}},90_000)}});
 });
 return io;
}
