...
const origins=new Set([...config.WEB_ORIGIN.split(',').map(origin=>origin.trim()).filter(Boolean),'http://localhost:5173','http://127.0.0.1:5173']);
app.disable('x-powered-by');
app.use(helmet({contentSecurityPolicy:false}),cors({origin:(origin,done)=>!origin||origins.has(origin)||/^https:\/\/[a-z0-9-]+\.pages\.dev$/i.test(origin)?done(null,true):done(new Error('CORS_ORIGIN_DENIED')),credentials:true}),express.json({limit:'100kb'}),cookieParser(),telemetry,rateLimit(300,60_000));
