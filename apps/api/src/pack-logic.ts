...
export const limitedRatePerCard=.5;
...
export function pickPackRarity(code:string,r:number,e:number,l:number){if(l>=40)return'LEGENDARY';const rates=ratesFor(code),entries=(['CELESTIAL','MYTHIC','ANCIENT','LEGENDARY','EPIC','RARE','UNCOMMON','COMMON']as const).map(name=>[name,rates[name]]as const);let cursor=0;const target=r*100;for(const[name,weight]of entries){let adjusted=weight;if(name==='EPIC')adjusted+=Math.max(0,e-11);cursor+=adjusted;if(target<cursor)return name}return'COMMON'}
