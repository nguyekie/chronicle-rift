...
describe('card effects',()=>{
 ...
 it('Foresee reveals the top card and waits for the player choice',()=>{let state=withCard(unit('seer',['Foresee']));const oldTop=state.players[0].deck[0]!.instanceId;state=summon(state);expect(state.pendingForesee).toMatchObject({playerId:'a',cardInstanceId:oldTop});expect(state.events.some(event=>event.type==='FORESEE_REVEALED')).toBe(true)});
 it('Foresee can move the revealed card to the bottom',()=>{let state=summon(withCard(unit('seer',['Foresee'])));const oldTop=state.players[0].deck[0]!.instanceId;state=applyAction(state,{type:'RESOLVE_FORESEE',playerId:'a',choice:'BOTTOM'}).state;expect(state.players[0].deck.at(-1)!.instanceId).toBe(oldTop);expect(state.pendingForesee).toBeUndefined()});
 it('Foresee can keep the revealed card on top',()=>{let state=summon(withCard(unit('seer',['Foresee'])));const oldTop=state.players[0].deck[0]!.instanceId;state=applyAction(state,{type:'RESOLVE_FORESEE',playerId:'a',choice:'KEEP'}).state;expect(state.players[0].deck[0]!.instanceId).toBe(oldTop);expect(state.pendingForesee).toBeUndefined()});
 ...
 it('buffed Limited grand spell splits sixteen damage, draws three, and restores energy',()=>{const grand:EngineCard={id:'limited-grand',name:'limited-grand',description:'Đại phép: gây 16 sát thương chia đều cho mọi kẻ địch, sau đó rút 3 lá và nhận 2 Năng lượng.',type:'SPELL',cost:1,keywords:[],damage:16};let state=withCard(grand);for(let index=0;index<2;index++){const enemy=state.players[1].hand[index]!;enemy.currentHealth=12;enemy.row='FRONT';state.players[1].board.FRONT.push(enemy)}const handBefore=state.players[0].hand.length;state=applyAction(state,{type:'PLAY_CARD',playerId:'a',cardInstanceId:state.players[0].hand[0]!.instanceId}).state;expect(state.players[1].board.FRONT.map(card=>card.currentHealth)).toEqual([4,4]);expect(state.players[0].hand.length).toBe(handBefore-1+3);expect(state.players[0].energy).toBe(11)});
});
